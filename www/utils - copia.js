...
function MediaLibrary(g) {
  ...
  cont = 0;
  levels[cont++] = { tipoghost: 0, numpajaros: 1, bg: 'f1.jpg', txt: 'La Alhambra de Granada', fuel: 200, traptipo: 0, speed: 0.1 };
  levels[cont++] = { tipoghost: 0, numpajaros: 3, bg: 'f1.jpg', txt: 'La Alhambra de Granada', fuel: 200, traptipo: 0, speed: 1 };
      ...
      function getanim(idx) {
        ...
        function movearrastrar() {
            if (ghost.movesteps > 0) {
                ...
                if (ghost.movesteps <= 0 && parseInt(mouse.x) != mouse.antx && parseInt(mouse.y) != mouse.anty) {
                    ...
                    var disty = mouse.y - (ghost.yPos + (ghost.h * ratiow / 2));
                    var distx = mouse.x - (ghost.xPos + (ghost.w * ratioh / 2));
                ...
                ghost.incy = (disty / ghost.movesteps)
                  ...
                  function rayo2(ctx) {
                      if (mouse.x < realw && mouse.y < realh) {
                          ...
                          ctx.lineTo(realw / 2 + desp2, realh - (ghost.h * ratioh) - 3);
        ...
        function rayo(ctx) {
            if (mouse.x < realw && mouse.y < realh) {
                var r1xold, r1yold;
                ctx.lineWidth = 25 * ratiow;
                r1xold = ghost.xPos + (ghost.w / 2 * ratiow);
                
                r1yold = ghost.yPos + (ghost.h / 2* ratioh);
                

                var pasoy = Math.round(Math.abs((ghost.yPos - (realh - (ghost.h*ratioh) - 3))) / 20);
        ...
        function testclick() {
            if (contclick >= 1) {
                ...
                if (mouse.x < ghost.xPos + ghost.w * ratiow && mouse.x > ghost.xPos) {
                    if (mouse.y < ghost.yPos + ghost.h * ratioh && mouse.y > ghost.yPos) {
                        gamestatus = 5;
                        resetmove2();
                    }
                }
            ...
            */
            ...
            if (pause != true) {
                if (gamestatus == 0) {
                    ...
                    mydrawimage(player.div, getanim(2), parseInt((realw / 2) - (player.w / 2) * ratiow), parseInt(realh - player.h * ratioh - 3), player.w, player.h);
                    if (loadingweapon <= 0) {
                        testclick();
                    }
                    if (paintRecal) {
                    ...
                    mydrawimage(player.div, getanim(2), parseInt((realw / 2) - (player.w / 2) * ratiow), parseInt(realh - player.h * ratioh - 3), player.w * ratiow, player.h * ratioh);

                    if (trap.estado == 0) {
                        mydrawimage(trap.div, getanim(8), trap.xPos, realh - trap.h * ratioh, trap.w, trap.h);
                    } else {
                        mydrawimage(trap.div, getanim(7), trap.xPos, realh - trap.h * ratioh, trap.w, trap.h);
                    }

                    mydrawimage(ghost.div, getanim(1), parseInt(ghost.xPos), parseInt(ghost.yPos), ghost.w, ghost.h);
                    mydrawimage(player.div, getanim(2), parseInt(ghost.xPos - (player.w / 2 * ratiow)), parseInt(realh - player.h * ratioh - 3), player.w, player.h);
                    ...
                    if (trap.estado == 1 && ghost.yPos + (ghost.h * ratioh) > realh - (ghost.h * ratioh) && ghost.xPos + ghost.w * ratiow >= trap.xPos && ghost.xPos <= trap.xPos + trap.w * ratiow) {
                        gamestatus = 2;
                ...
                if (gamestatus == 2) {
                    ...
                    mydrawimage(player.div, getanim(2), ghost.xPos - (player.w / 2 * ratiow), realh - (player.h * ratioh) - 3, player.w, player.h);
                ...
                if (gamestatus == 5) {
                    move2();
                    mydrawimage(ghost.div, getanim(1), ghost.xPos, ghost.yPos, ghost.w, ghost.h);
                    mydrawimage(player.div, getanim(2), ghost.xPos - (player.w * ratiow / 2), realh - (player.h * ratioh) - 3, ghost.w, ghost.h);
                    //window.setTimeout(doAnimation, clockspeed);
                    rayos();
                ...
                if (gamestatus == 9) {
                    ...
                    var explo = getanim(4)
                    mydrawimage(player.div, ML.sprites["aplayerfin"], ghost.xPos - (ghost.w / 2 * ratiow), realh - (player.h * ratioh) - 3, ghost.w, ghost.h);
                    mydrawimage(explodiv.div, explo, ghost.xPos - (ghost.w / 2 * ratiow), realh - (player.h * ratioh) - 3, ghost.w * 2, ghost.h * 2);
